			...
			for (const node of mutation.addedNodes) {
				if (node.tagName === "TD") {
						...
						if (
							...
							const itemInfoInterval = setInterval(() => {
								...
								const nonCraftable = [
									...
									elem =>
										elem.innerText.trim() === "Craftable"
								);
								if (nonCraftable)
									item = `Non-Craftable ${item}`;
								item = encodeURIComponent(item);
								const newLink = link.cloneNode();
								newLink.innerHTML = ICON(15, 15);
								const at = new Date(
									parseInt(href.split("?time=")[1]) * 1000
								);
								newLink.setAttribute(
									"href",
									`https://gladiator.tf/time-machine?item=${item}&at=${at.toISOString()}`
								);
					...
					for (const card of cards) {
						if (!card.getElementsByClassName("vote-buttons").length)
							continue;

						item = card.getElementsByClassName(
							"card__header__title"
						)[0].innerText;
					}
					if (!item) continue;
					...
					if (node.getElementsByClassName("btn-item-glad").length)
						continue;
					...
					statsLink.setAttribute(
						"href",
						`https://gladiator.tf/sales?item=${item}&at=${new Date().toISOString()}`
					);
	...
	if (location.pathname.startsWith("/suggestion")) {
		...
		$(".history-sheet tr").each(function () {
			...
			tr.find("td:nth-child(2)").append(
				`<span style="float: right; margin-left: 0.6em;"><a href="https://gladiator.tf/time-machine?item=${item}&at=${time.toISOString()}" target="_blank" data-tip="bottom" title="Gladiator.tf Time Machine"><i class="fa fa-clock-o fa-fw"></i></a></span>`
			);
		});
	}
}
		...
		const id = setInterval(function () {
			...
			if ($element.hasClass("popover")) {
				...
				if ($gladLinks.length === 0) {
					const $additionalLinks = $element.find(
